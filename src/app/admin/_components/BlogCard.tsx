...
//   // Handle card click to view blog
...
//   return (
//     <div 
//       className="group h-full bg-slate-800/50 backdrop-blur-xl rounded-2xl border border-slate-700/50 overflow-hidden transition-all duration-500 transform hover:-translate-y-2 hover:shadow-2xl hover:shadow-blue-500/10 cursor-pointer"
...
//           <button
//             onClick={handleEdit}
//             className="group/btn flex items-center gap-2 px-3 py-2 text-blue-400 hover:text-blue-300 hover:bg-blue-500/10 font-medium text-sm rounded-lg transition-all duration-300 z-10"
//           >
//             <Edit2 className="w-4 h-4 group-hover/btn:scale-110 transition-transform duration-300" />
//             <span>Edit</span>
//           </button>
...
//           <button
//             onClick={handleView}
//             className="group/btn flex items-center gap-2 px-3 py-2 text-slate-400 hover:text-slate-300 hover:bg-slate-700/50 font-medium text-sm rounded-lg transition-all duration-300 z-10"
//           >
//             <Eye className="w-4 h-4 group-hover/btn:scale-110 transition-transform duration-300" />
//             <span>Preview</span>
//           </button>
...
//             <Link
//               href={`/blog/${blog.slug}`}
//               className="group/btn flex items-center gap-2 px-3 py-2 text-green-400 hover:text-green-300 hover:bg-green-500/10 font-medium text-sm rounded-lg transition-all duration-300 z-10"
//               target="_blank"
...
//       {/* Click indicator */}
//       <div className="absolute bottom-2 right-2 opacity-0 group-hover:opacity-60 transition-opacity duration-300 pointer-events-none">
//         <div className="text-xs text-slate-400 bg-slate-800/80 px-2 py-1 rounded-md backdrop-blur-sm">
//           Click to preview
//         </div>
//       </div>
//     </div>
...
export default function BlogCard({ blog, allBlogs, setBlogs }) {
  ...
  // Handle card click to view blog
  const handleCardClick = (e) => {
    // Prevent card click if clicking on buttons or links
  ...
  const handleDeleteBlog = async (e, id) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(`Failed to delete blog: ${errorData.error || 'Please try again.'}`);
      }
    } catch (error) {
      console.error('Error deleting blog:', error);
      alert('An error occurred while deleting the blog. Please check your connection and try again.');
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="relative h-48 overflow-hidden">
        ...
        {/* Status and Action Badges */}
        <div className="flex items-center justify-between w-full absolute top-4 px-4 z-20">
          <div>
            <span className={`px-3 py-1.5 rounded-full text-xs font-bold backdrop-blur-sm border transition-all duration-300 cursor-default ${
              blog.published
                ? 'bg-green-500/20 text-green-300 border-green-500/30 shadow-lg shadow-green-500/10'
