...
import { useParams, useRouter } from 'next/navigation';
import { motion } from 'framer-motion';
import { ArrowLeft, Calendar, Clock, List, Share2, Facebook, Twitter, Linkedin, Copy, CheckCircle, BookOpen } from 'lucide-react';

// Blog styles (updated for dark theme)
const blogStyles = `
  .blog-content {
    font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;
    color: #e2e8f0;
      ...
      <div className="min-h-screen bg-[#1a1f3a] text-white">
        ...
        <div className="relative z-10 flex items-center justify-center min-h-screen">
          <div className="text-center">
            ...
            <p className="text-slate-400 text-lg">Fetching your content</p>
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="min-h-screen bg-[#1a1f3a] text-white">
        ...
        <div className="relative z-10 max-w-4xl mx-auto p-6 pt-24">
          <div className="bg-red-500/20 border border-red-500/30 p-8 rounded-2xl text-red-300 shadow-2xl backdrop-blur-xl">
      ...
      <div className="min-h-screen bg-[#1a1f3a] text-white">
        ...
        <div className="relative z-10 flex items-center justify-center min-h-screen p-6">
          <motion.div 
            ...
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.5 }}
            className="max-w-xl w-full bg-yellow-500/20 border border-yellow-500/30 p-8 rounded-2xl shadow-2xl backdrop-blur-xl text-center"
          >
            <div className="w-20 h-20 bg-gradient-to-br from-yellow-500/20 to-orange-600/20 rounded-2xl flex items-center justify-center mx-auto mb-6">
              <BookOpen className="w-10 h-10 text-yellow-400" />
            </div>
            <h2 className="text-3xl font-bold mb-4 text-yellow-300">This blog post is not published yet</h2>
            <p className="text-yellow-200/80 mb-8 leading-relaxed">
              This blog is still in draft mode and is not available to the public. 
              Only authors and administrators can view drafts.
            </p>
            <div className="flex gap-4 justify-center">
              <Link 
                href="/blog" 
                className="px-6 py-3 bg-slate-700/50 hover:bg-slate-700/70 text-slate-300 hover:text-white rounded-xl transition-all border border-slate-600/50"
              >
                Back to Blogs
              </Link>
            </div>
    ...
    <div className="min-h-screen bg-[#1a1f3a] text-white">
      ...
      <div className="relative z-10 max-w-7xl mx-auto p-6 pt-24 pb-16">
        ...
        >
          <Link 
            href="/blogs" 
            className="flex items-center gap-3 text-slate-300 hover:text-white font-medium transition-colors"
          >
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          >
            {blog && (
              <>
                ...
                {/* Share buttons (mobile - top) */}
                <div className="lg:hidden bg-slate-800/50 backdrop-blur-xl rounded-2xl border border-slate-700/50 p-6 mb-8 shadow-2xl">
                  ...
                  <div className="flex gap-3">
                    <button
                      onClick={() => shareOnSocial('facebook')}
                      className="bg-[#1877f2] hover:bg-[#1877f2]/80 text-white p-3 rounded-xl transition-colors shadow-lg"
                      aria-label="Share on Facebook"
                    >
                      <Facebook size={20} />
                    </button>
                    <button
                      onClick={() => shareOnSocial('twitter')}
                      className="bg-[#1da1f2] hover:bg-[#1da1f2]/80 text-white p-3 rounded-xl transition-colors shadow-lg"
                      aria-label="Share on Twitter"
                    >
                      <Twitter size={20} />
                    </button>
                    <button
                      onClick={() => shareOnSocial('linkedin')}
                      className="bg-[#0a66c2] hover:bg-[#0a66c2]/80 text-white p-3 rounded-xl transition-colors shadow-lg"
                      aria-label="Share on LinkedIn"
                    >
                      <Linkedin size={20} />
                    </button>
                    <button
                      onClick={copyToClipboard}
                      className="bg-slate-700/50 hover:bg-slate-700/70 text-slate-300 hover:text-white p-3 rounded-xl transition-colors relative shadow-lg border border-slate-600/50"
                      aria-label="Copy link"
                    >
                      {copied ? <CheckCircle size={20} className="text-green-400" /> : <Copy size={20} />}
                      {copied && (
                        <span className="absolute -bottom-8 left-1/2 transform -translate-x-1/2 bg-slate-800/90 text-white text-xs py-1 px-2 rounded whitespace-nowrap border border-slate-700/50">
                          Copied!
                        </span>
                      )}
                    </button>
          ...
          >
            <div className="sticky top-24 space-y-6">
              {/* Share section */}
              <div className="bg-slate-800/50 backdrop-blur-xl p-4 rounded-2xl border border-slate-700/50 shadow-2xl relative">
                <div className="flex items-center justify-between">
                  <button 
                    onClick={() => setShowShareOptions(!showShareOptions)}
                  ...
                  <div className="flex gap-2">
                    <button 
                      onClick={() => shareOnSocial('facebook')} 
                      className="w-8 h-8 rounded-full bg-[#1877f2] hover:bg-[#1877f2]/80 text-white flex items-center justify-center transition-colors"
                    >
                      <Facebook size={15} />
                    </button>
                    <button 
                      onClick={() => shareOnSocial('twitter')} 
                      className="w-8 h-8 rounded-full bg-[#1da1f2] hover:bg-[#1da1f2]/80 text-white flex items-center justify-center transition-colors"
                    >
                      <Twitter size={15} />
                    </button>
                    <button 
                      onClick={() => shareOnSocial('linkedin')} 
                      className="w-8 h-8 rounded-full bg-[#0a66c2] hover:bg-[#0a66c2]/80 text-white flex items-center justify-center transition-colors"
                    >
                      <Linkedin size={15} />
                    </button>
                    <button 
                      onClick={copyToClipboard} 
                      className="w-8 h-8 rounded-full bg-slate-700/50 hover:bg-slate-700/70 text-slate-300 hover:text-white flex items-center justify-center transition-colors border border-slate-600/50"
                    >
                ...
                {showShareOptions && (
                  <div className="absolute top-full left-0 right-0 mt-2 bg-slate-800/95 backdrop-blur-xl p-4 rounded-xl shadow-2xl z-10 space-y-2 border border-slate-700/50">
                    <button
                      onClick={() => shareOnSocial('facebook')}
                      className="w-full flex items-center gap-3 text-left p-3 rounded-lg hover:bg-slate-700/50 transition-colors text-slate-300 hover:text-white"
                    >
                      <Facebook size={18} className="text-[#1877f2]" />
                      <span>Share on Facebook</span>
                    </button>
                    ...
                    <ul className="space-y-3">
                      {blog.tocItems.map((item, index) => (
                        <li 
                          key={index}
                          className={`
                            ${item.level === 1 ? 'font-medium text-base border-l-4 border-blue-400 pl-4' : 
